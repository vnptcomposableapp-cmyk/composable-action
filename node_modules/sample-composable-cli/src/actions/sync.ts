...
export async function sync(
  ...
  logger.info(`[opts]: ${JSON.stringify(opts, null, 2)}`);
  ...
  logger.info('[projectsToSync] pre');
  ...
  logger.info('[projectsToSync] post');
  ...
  const doSync = async () => {
    ...
    logger.info('[fixAllImportStatements] pre');
    ...
    logger.info('[fixAllImportStatements] post');

    logger.info('[codegenVersion] pre');
    const codegenVersion = await context.api.latestCodegenVersion();
    logger.info('[codegenVersion] post');
    ...
    logger.info('[updateConfig] pre');
    await updateConfig(context, context.config, baseDir);
    logger.info('[updateConfig] post');
  };

  // Perform the actual sync
  if (opts.skipBuffering) {
    logger.info('[opts.skipBuffering] pre');
    await doSync();
    logger.info('[opts.skipBuffering] post');
  } else {
    logger.info('[withBufferedFs] pre');
    await withBufferedFs(doSync);
    logger.info('[withBufferedFs] post');
  }
...
async function syncProject(
  ...
  logger.info('[existingChecksums]');

  // Server-side code-gen
  logger.info('[projectBundle] pre');
  ...
  logger.info(`[projectBundle] post: ${JSON.stringify(projectBundle)}`);
