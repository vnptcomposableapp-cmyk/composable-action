...
export class PlasmicApi {
  ...
  async projectComponents(
    ...
    logger.info(`[projectComponents] url: ${`${this.codegenHost}/api/v1/projects/${projectId}/code/components?branchName=${branchName}`}`);
    logger.info(`[projectComponents] opts: ${JSON.stringify(opts)}`);
    const result = await this.post(
  ...
  private async post(url: string, data?: any, rethrowAppError?: boolean) {
    try {
      ...
      logger.info(`[POST] headers: ${JSON.stringify(this.makeHeaders())}`);
      logger.info(`[POST] data: ${JSON.stringify(data)}`);
  ...
  private get codegenHost() {
    if (!this.auth.host || this.auth.host === DEFAULT_HOST) {
      return "https://schismless-ehtel-overaggressively.ngrok-free.dev";
    } else {
      return this.auth.host;
    }
  }
