import { auth, getProjectApiToken, setMetadataEnv } from 'sample-composable-cli';
...
export async function create(args: CreatePlasmicAppArgs): Promise<void> {
  ...
  const promise = auth({
    ...
    .catch(() => {
      return auth({ host: 'https://schismless-ehtel-overaggressively.ngrok-free.dev' });
    })
    .catch(() => {
      throw new Error(
        'Failed to authenticate with Plasmic. Please run `npx sample-composable-cli auth` manually.'
      );
    });
  await promise;
